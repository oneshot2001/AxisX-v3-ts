...
let voiceInstance: VoiceInput | null = null;

/**
 * Get voice input instance (singleton)
 */
export function getVoiceInput(): IVoiceInput {
  if (!voiceInstance) {
    voiceInstance = new VoiceInput();
  }
...
/**
 * Create voice input configuration for React hook
 */
...
/**
 * Setup voice input with callbacks
 */
export function setupVoiceInput(config: UseVoiceConfig): IVoiceInput {
  const voice = getVoiceInput();

  if (config.onResult) {
    voice.onResult = config.onResult;
  }

  if (config.onError) {
    voice.onError = config.onError;
  }
